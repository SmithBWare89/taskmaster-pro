...
// Add click event listener to blur the text area
...
function auditTask(taskEl){
  ...
  // "L" sets the date to local time
  var time = moment(date, "L").set("hour", 17);
  console.log(time);

  // remove old clases from element
  $(taskEl).removeClass("list-group-item-warning list-group-item-danger");

  // apply new class if task is near/over the value or time or 5:00PM
  if (moment().isAfter(time)) {
    $(taskEl).addClass("list-group-item-danger");
  } else if (Math.abs(moment().diff(time, "dates")) <= 2) {
